...
if __name__ == "__main__":
    ...
    # Plot for markers
    st_data     = "stdata_10X.csv"
    loc_data    = "st_location_10X.csv"
    ...
    # Plot for marker
    axis[0, 0].set_title("Synpo2, Ptpn3, Slc17a6", size=20)
    ...
    cell_type_name = ["Ext_Thal_1", "Ext_Thal_2"]
    cell_type_prop = numpy.zeros(X.shape)
    for name in cell_type_name:
        cell_type_prop += df_pred[name].to_numpy()
    # Plot
    axis[0, 1].set_title("Berglund\nCorr: {}".format(round(stats.pearsonr(cell_type_prop, gene_value)[0], 2)), size=20)
    ...
    pred_data = "DSTG.csv"
    df_pred = pd.read_csv(pred_data, header=0, index_col=0)
    cell_type_name = ["Ext.Thal.1", "Ext.Thal.2"]
    cell_type_prop = numpy.zeros(X.shape)
    for name in cell_type_name:
        cell_type_prop += df_pred[name].to_numpy()
    # Plot
    ...
    df_pred = pd.read_csv(pred_data, header=0, index_col=0)
    df_pred = pd.DataFrame.transpose(df_pred)
    cell_type_name = ["Ext_Thal_1", "Ext_Thal_2"]
    cell_type_prop = numpy.zeros(X.shape)
    for name in cell_type_name:
        cell_type_prop += df_pred[name].to_numpy()
    ...
    # Checkpoint
    plt.subplots_adjust(left=0.1, bottom=0.1, right=0.9, top=0.9, wspace=0.4, hspace=0.6)
    plt.savefig("10X_case3.png", dpi=1200)
    exit()
